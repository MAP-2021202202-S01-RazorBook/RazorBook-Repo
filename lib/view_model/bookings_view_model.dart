...
//result is YYYY-MM-DD of the working day
  ...
  Future getBookings() async {
    ...
    try {
      ...
      // }
      await bookingService.getCustomerBookings(
          userID: currentUser?.u_id ?? "3OS5MJ1kAFYfMpdq4R8odxJtsvl2");
      bookings = bookingService.customerBookingsList;
...
//This is for now is hardcoded as we don't have anything to store the loggin information

  Future makeBooking(BuildContext ctx) async {
    Booking booking = Booking(
      c_id: "3OS5MJ1kAFYfMpdq4R8odxJtsvl2", // auth user id  not (users.docs.id)
      b_id: "Ftnf3iYjVyNiFxiFpwmHjcUmoqf2",
      is_cancelled: false,
      services:
          services.where((element) => element['isSelected'] == true).toList(), // need json encode 
    ...
    try {
      await bookingService
          .makeBooking(booking)
          .then((value) => ScaffoldMessenger.of(ctx).showSnackBar(mySnackBar(
                'Booking Successful',
              )));
    } catch (e) {
